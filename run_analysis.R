####################################################################################################3#
# Function run_analysis - Implementation of programming assignment - Geting and Cleanind Data
# Author: Jorge Gervasio Pereira   Date (Final Versio): October 24, 2015
#
#
#
...
##########################################################################################################
# Group Local Data Frame by Activity Names and Subject ID columns and sumarize only the columns with
# Standard Deviation and Mean of the signals colected from Samsung Galaxy Mobile phones

Tidy_Data <- Nodup_Data_Merged %>% group_by(ActivityNames,SubjectID) %>% summarise_each(funs(mean),one_of(Vector_to_Be_Analyzed))
...
write.table(Tidy_Data,file = "C:/Users/USUARIO PC/Documents/Coursera- Getting and Cleaning Data/UCI HAR Dataset/Tidy_Data.txt",col.names = TRUE,row.names = FALSE)
